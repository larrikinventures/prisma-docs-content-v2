...
![](https://imgur.com/jPLMaF5.png)

> **Notes**
>
> - If you've seen a service deployed to an endpoint that didn't have any path components (e.g. `http://localhost:4466`), it uses the `default` values for service name and stage. This means **`http://localhost:4466/default/default` can always be written as `http://localhost:4466`**.
...
See here for more info about [Prisma services](tbd).
...
You can think of the data model as a way to specify your *database schema* in a declarative way. (Prisma enforces a schema even for schemaless databases.)

> ⚠️  Don't confuse the data model with a GraphQL schema.
