...
More use cases for Prisma are scripts, functions, queue workers or any other  context where developers need to talk to a database. 

When building a GraphQL server that uses Prisma as its data access layer, it is important to understand that the auto-generated Prisma GraphQL API is *not* consumed by your client applications - instead you build the actual *application server* on top of Prisma.
...
- ... **an ORM** - Although it solves similar problems as ORMs (more [here](https://github.com/prismagraphql/prisma#is-prisma-an-orm)).
...
_Prisma_ is open-source and can be used as a standalone infrastructure component hosted on your favorite cloud provider. _Prisma Cloud_ is an application (used through a CLI & web interface) that provides tools and services around Prisma. When you're using Prisma, usage of Prisma Cloud is optional.
...
- **Auto-generated CRUD GraphQL API** for easy database access.
