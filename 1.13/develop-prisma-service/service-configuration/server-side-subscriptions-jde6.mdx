...
Server-side subscriptions enable you to invoke webhooks whenever a _subscription event_ fires in your Prisma API. They are configured in your service's prisma.yml and are using the same subscription API that's defined by your Prisma database schema (learn more [here](qwe3)).
...
Note that [batch operations](vw4d#batch-operations) are not triggering subscription events.
