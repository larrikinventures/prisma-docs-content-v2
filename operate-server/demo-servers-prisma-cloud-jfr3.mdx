...
This command opens a browser window where you need to provide your Prisma Cloud credentials. If the credentials are valid, the CLI generates a Prisma Cloud token and stores it in `~/.prisma/config.yml` as `cloudSessionKey`.
