...
The `Mutation` type of the Prisma database schema defines all the mutations the Prisma API accepts.
...
- **Object mutations**: Create, update, upsert or delete a single node of a certain [object type](!alias-eiroozae8u#object-types).
- **Nested mutations**: Create, update, upsert, delete, connect, disconnect multiple nodes of two or more related types.
- **Batch mutations**: Update and delete many nodes of a certain [object type](!alias-eiroozae8u#object-types).
...
You can change the values of one or more fields of a node using an `update` mutation. The node to be updated is [selected](!alias-utee3eiquo#node-selection) using the `where` argument and the new values are provided via the `data` argument. Nodes can be selected by any field that's annotated with the `@unique` directive. 
...
To delete nodes, you need to [select the node](!alias-utee3eiquo#node-selection) to be deleted via the `where` argument in a `delete` mutation.
...
You can use `create` and `update` mutations to modify nodes across relations at the same time. This is referred to as **nested mutations** and is executed [transactionally](!alias-utee3eiquo#transactional-mutations).
...
> **Note**: `push`, `pop` and `remove` are not yet implemented. If you're curios what these are going to look like, you can get a preview in the respective [specification](https://github.com/prismagraphql/prisma/issues/1275).
...
Batch mutations are **not** triggering [subscription](!alias-aey0vohche) events!
