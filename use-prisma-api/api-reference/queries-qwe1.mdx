...
The GraphQL API of a Prisma service is specified in the Prisma database schema. The Prisma database schema is auto-generated based on the service's data model:

![](https://imgur.com/ZIjHLpL.png)

The `Query` type of the Prisma database schema defines all the queries the Prisma API accepts.
...
This is what the generated `Query` type looks like:
...
For every type in your data model, three queries are generated:
...
To inspect all the available operations of your Prisma API in detail, you can read the Prisma database schema of your Prisma service.It can be downloaded with the [GraphQL CLI](https://github.com/graphql-cli/graphql-cli): 
...
* **Object queries**: Fetch _single_ or _multiple_ nodes of a certain [object type](!alias-eiroozae8u#object-types).
...
> Note that more aggregations will be added over time. Find more information about the roadmap [here](https://github.com/prismagraphql/prismagraphql/issues/1312).
...
> WARNING: It's currently not possible to order responses [by multiple fields](https://github.com/prismagraphql/feature-requests/issues/62) or [by related fields](https://github.com/prismagraphql/feature-requests/issues/95). Join the discussion in the feature requests if you're interested in these features!
...
![](https://imgur.com/e2UnoWJ.png)

**Query the last 4 nodes (seeking _backward_)**:
...
![](https://imgur.com/C32uDLG.png)

**Query the nodes from position 20 to position 26 (seeking _backward_):**
...
![](https://imgur.com/Ke04CCc.png)

**Query the first 2 nodes after the node with `cixnen24p33lo0143bexvr52n` as `id`:**
...
![](https://imgur.com/7NAPqkX.png)

**Query the first 4 nodes after the node with `cixnen24p33lo0143bexvr52n` as `id` and skipping 3 nodes:**
...
![](https://imgur.com/8nfeLPv.png)

**Query the last 6 nodes before the node with `cixnen24p33lo0143bexvr52n` as `id`:**
