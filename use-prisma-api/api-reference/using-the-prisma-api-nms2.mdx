...
Here is the structure of an HTTP request that carries a `createUser` mutation:

**Header**

| Field | Sample Value | Description | 
| --- | --- | --- |
| `Authorization` | `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjp7InNlcnZpY2UiOiJhc2RAZGV2Iiwicm9sZXMiOlsiYWRtaW4iXX0sImlhdCI6MTUzMjUyMjc3NywiZXhwIjoxNTMzMTI3NTc3fQ.BJfWMlNhMTaXC4ZYZHx5J6vSnmhg9ddMFwuoYmu3w9I` | Carries the service token used to authenticate a request; only required if the service is deployed with a service secret |
| `Content-Type` | `application/json` | Specifies the format of the request's body (JSON)  |

**Body**

| Field | Sample Value | Description | 
| --- | --- | --- |
| `query` | `mutation($name: String!) { createUser(data: { name: $name }) { id } }` | The GraphQL operation to be sent to the API; beware that despite the field being called `query` it's also used for mutations |
| `variables` | `{ name: "Sarah" }` | The variables defined in the GraphQL operation |
...
-H 'Authorization: Bearer __YOUR_PRISMA_TOKEN__' \\
--data-binary '{"query":"mutation ($name: String!) { createUser(data: { name: $name }) { id } }","variables":{"name":"Sarah"}}'
```

To **try out this example**, replace the `__YOUR_PRISMA_ENDPOINT__` and `__YOUR_PRISMA_TOKEN__` placeholders with corresponding values from your Prisma service and paste the resulting snippet into your terminal.
...
1. Replace the `__YOUR_PRISMA_ENDPOINT__` and `__YOUR_PRISMA_TOKEN__` placeholders with corresponding values from your Prisma service
...
> WARNING: `request` does not support passing _headers_ to the request ([yet](https://github.com/prismagraphql/graphql-request/pull/91)). Therefore, this particular example assumes your Prisma service was deployed without a service secret.
...
const client = new GraphQLClient('__YOUR_PRISMA_ENDPOINT__', {
  headers: {
    Authorization: 'Bearer __YOUR_PRISMA_TOKEN__',
  },
})
