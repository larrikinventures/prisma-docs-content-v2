...
- `Authorization`
  ...
  - **Description:** Carries the service token used to authenticate a request; only required if the service is deployed with a service secret.
- `Content-Type`
  - **Sample value:** `application/json`
  - **Description:** Specifies the format of the request's body (JSON).
...
- `query`
  - **Sample value**: 
    ...
    ```js
    "mutation($name: String!) { createUser(data: { name: $name }) { id } }"
    ```
  - **Description:** The GraphQL operation to be sent to the API; beware that despite the field being called `query` it's also used for mutations.
- `variables`
  - **Sample value**: `"{ name: "Sarah" }"`
  - **Description:**  The variables defined in the GraphQL operation.
