...
**Update `published` of a many `Post` nodes at once** (also see [Batch operations](#batch-operations)):
...
mutation {
  deleteManyUsers(
    where: {
      id_in: ["alice@prisma.io", "bob@prisma.io"]
    }
  ) {
    count
  }
...
Single mutations in the Prisma API that are not batch operations are always executed _transactionally_, even if they consist of many actions that potentially spread across multiple related nodes. This is especially useful for [nested mutations](!alias-ol0yuoz6go#nested-mutations) that perform several database writes on multiple types.
...
> WARNING: The service secret and service tokens are not to be confused with the Prisma Management API secret used to protect the Management API of your Prisma server.