...
Notice that all model types in the Prisma database schema will implement the `Node` interface. This is what the `Node` interface looks like:
...
Nodes for a type that contains a required _to-one_ relation field can only be created using a [nested mutation](!alias-ol0yuoz6go#nested-mutations) to ensure the respective field will not be `null`.
...
The relations that are included in your schema affect the available operations in the [Prisma API](!alias-abogasd0go). Here is an overview of the generated CRUD and realtime operations for every relation in your Prisma API: 
...
- [Nested mutations](!alias-ol0yuoz6go#nested-mutations) allow you to create, connect, update, upsert and delete multiple related nodes within the same mutation.
