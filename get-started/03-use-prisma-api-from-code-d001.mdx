...
> The Prisma API is a GraphQL API that's served via HTTP. You can therefore consume it using any HTTP tool you like, such as `curl` or JavaScript's `fetch`. On this page, you're using Prisma bindings which provide a convenient abstraction on top of `fetch` to reduce boilerplate and improve the developer experience when working with Prisma. 
...
## Step 2: Install Prisma bindings
...
## Step 3: Download the GraphQL schema of your Prisma API
...
## Step 3: Instantiate Prisma API using Prisma bindings
...
## Step 4: Instantiate a `Prisma` binding
...
## Step 5: Use your Prisma binding to send queries and mutations
...
1. Fetch the newly created `User` using the `user` query (**line 18**):
  ...
  > TODO: Collapse and make the explanations expandable
